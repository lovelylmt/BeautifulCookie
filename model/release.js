...
module.exports = {
    //用户上传菜谱
    addRecipe: function (recipeName, img666, stepImg1, stepImg2, stepImg3, recipeDescription, mainMaterial, mainMaterialUsage, accessories, accessoriesUsage, step1, step2, step3, username, status, userIcon, recipeType, cb) {
    ...
    //管理员发布审核的菜谱
    upRecipe: (cb) => {
        recipename.find({ status: "1", }).then(result => { console.log(result); cb(result) })
    },

    //具体菜谱做法
  
    cookdetail:  (recipeName, cb) => { console.log(recipeName)
        recipename.findOne({                            
            recipeName: recipeName
